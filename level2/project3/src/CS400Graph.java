...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			this.start = source.start; // copy the start point
			this.distance = source.distance; // copy the total distance of the path
			this.dataSequence = new LinkedList<>(); // create a new list
			for (T data : source.dataSequence) // use for loop to copy every element
				this.dataSequence.add(data);
			this.end = source.end; // copy the end point
		}
		...
		public void extend(Edge edge) {
			this.distance += edge.weight; // this distance update
			this.dataSequence.add(edge.target.data); // this dataSequence update
			this.end = edge.target;// end update
		}
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		if (start == null || end == null) // check the input data
			throw new NoSuchElementException();
		Vertex st = vertices.get(start); // convert the input data into a Vertex object
		Vertex en = vertices.get(end);
		if (st == null || en == null) // check the if vertices hashtable contains the data
			throw new NoSuchElementException();
		PriorityQueue<Path> pq = new PriorityQueue<Path>(); // create new Priority Queue to store path
		Hashtable<Vertex, Path> visited = new Hashtable<Vertex, Path>(); // create a new hashtable to track visited
																			// Vertex
		Path p = new Path(st); // use start vertex to create new path
		pq.add(p); // add into the queue
		Vertex current = st; // Initial the running c with the start vertex
		Path shortest = pq.poll();// Initial the shortest path with the start path
		while (!current.equals(en)) { // check if the current vertex is the end vertex
			for (Edge e : current.edgesLeaving) { // loop over all the leaving edge
				Path newpath = new Path(shortest); // copy the shortest path
				newpath.extend(e); // extend with the edge
				pq.add(newpath); // update the path with the new edge
			}
			if (pq.isEmpty()) // if the queue is empty, there is no new path anymore
				throw new NoSuchElementException();
			shortest = pq.poll(); // update the shortest queue
			while (visited.containsKey(shortest.end)) { // check all of the visited vertices
				shortest = pq.poll();
				if (shortest == null) // check if the queue is empty
					throw new NoSuchElementException(); // stop the loop, there is no connection between start and end
			}
			visited.put(current, shortest); // put current vertex into the the visited table
			if (!shortest.end.equals(en)) { // if the current not matches with the end vertex
				current = vertices.get(shortest.end.data); // update the current
			} else { // if the current matches with the end vertex
				return shortest; // return that path
			}
		}
		throw new NoSuchElementException();
	}

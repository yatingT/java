...
//      throw new NoSuchElementException("the start and end data must be a valid data");
//    }
//
...
//      throw new NoSuchElementException("the vertices cannot be found in the graph!");
//    }
//
//    try {
//      Path pathA = new Path(startVertex); // start = startVertex; end = endVertex; distance = 0
//      // record visited vertices
//      CS400Graph<T> visited = new CS400Graph<>();
//      // create a new riority queue
//      PriorityQueue<Path> record = new PriorityQueue<>();
//      PriorityQueue<Path> centerCollect = new PriorityQueue<>();
//
//      // add the first vertex into the queue
//      record.add(pathA);
//
//      while (!record.isEmpty()) {
//        // [c's total wt, c] = record.removeMin()
//        Path center = record.poll();
//        centerCollect.add(center);// record every center
//        Vertex remove = center.end;
//
//        // set c's visited mark to true
//        visited.insertVertex(remove.data);// set the vertex as visited
//
//        // iterate around each successor of the center vertex
//        for (Edge e : remove.edgesLeaving) {
//          // find the unvisited successor of the center vertex
//          if (!visited.containsVertex(e.target.data)) {
//            Path successor = new Path(center);
//            successor.extend(e);// a new path including successor
//
//            // if S's total distance can be reduced
//            record.add(successor);
//          }
//        }
//      }
//
//      for (Path p : centerCollect) {
//        if (p.end.data == endVertex.data) {
//          return p;
//        }
//      }
...
//    } catch (Exception e) {
//      System.out.println("Unexpected exeption occurs! ");
//    }
